...
384. **Top-level `export --help --output-format json` exits successfully but emits plain text help instead of JSON** — dogfooded 2026-04-30 for the 04:30 nudge on current `origin/main` / local workspace while the checked-out `./rust/target/debug/claw` binary still reported embedded `git_sha` `d95b230c`. A fresh probe on the session-export/distribution surface showed a prompt-but-wrong contract: `./rust/target/debug/claw export --help --output-format json` exited `0` with `stdout=378` and `stderr=0`, but stdout began with plain text `Export`, `Usage`, `Purpose`, `Defaults`, `Formats`, and `Related` rather than a JSON object. In the same workspace, `version --output-format json` returned promptly with version/build metadata, proving the binary itself and JSON output path are reachable. This is distinct from the zero-byte hang class (#358/#380/#381/#382/#383) and from earlier export path/extension semantics: the affected surface is help/discovery for top-level `export`, where claws need bounded machine-readable usage before serializing or moving session artifacts. **Required fix shape:** (a) make `export --help --output-format json` emit valid stdout JSON with `kind:"help"` or `kind:"export"`, `action:"help"`, usage, options, defaults, output formats, examples, side-effect/path semantics, and related slash/direct commands; (b) preserve text help for default/text mode only; (c) add a `format:"json"` or equivalent field so callers can assert the contract without parsing prose; (d) add regression coverage proving export help with JSON format parses as JSON and does not silently fall back to plain text. **Why this matters:** export is how claws archive, transfer, and review session state. If JSON help accepts the flag but returns prose, automation must scrape formatting-sensitive text before deciding where artifacts will be written or which output format is safe. Source: gaebal-gajae dogfood follow-up for the 04:30 nudge on rebuilt/local `./rust/target/debug/claw` `d95b230c`.
